...
def make_json_error(message, error_type='error', details=None, status_code=400):
    ...
    def post(self, request):
        ...
        if not allowed:
            return make_json_error(
                f'Too many coupon attempts. Please try again later. ({attempts}/{COUPON_RATE_LIMIT})',
                'throttled',
                status_code=429
            )
        ...
        try:
            ...
            if not code:
                return make_json_error('Coupon code is required', 'validation_error')
            
            if cart_total < 0:
                return make_json_error('Invalid cart total', 'validation_error')
            ...
            if coupon_data:
                try:
                    ...
                    return make_json_error('Invalid coupon code', 'validation_error')
            else:
                try:
            ...
            if not coupon.is_valid:
                return make_json_error('This coupon has expired or reached its usage limit', 'validation_error')
            
            can_use, msg = coupon.can_use(user_email)
            if not can_use:
                return make_json_error(msg, 'validation_error')
            ...
            return make_json_error(str(e), 'validation_error')
        except Exception as e:
            logger.error(f"Coupon application failed: {e}", exc_info=True)
            return make_json_error('Failed to apply coupon', 'server_error', status_code=500)
...
class RemoveCouponView(View):
    ...
    def post(self, request):
        try:
            ...
            logger.error(f"Remove coupon failed: {e}", exc_info=True)
            return make_json_error('Failed to remove coupon', 'server_error', status_code=500)
...
class ActiveOffersView(View):
    ...
    def get(self, request):
        try:
            ...
            if cached:
                return JsonResponse({'success': True, 'offers': cached})
            ...
            logger.error(f"Active offers failed: {e}", exc_info=True)
            return make_json_error('Failed to retrieve offers', 'server_error', status_code=500)
...
class SubmitReviewView(View):
    ...
    def post(self, request):
        if not request.user.is_authenticated:
            return make_json_error('Please login to submit a review', 'authentication_error', status_code=401)
        
        try:
            product_id = validate_id(request.POST.get('product_id'))
            if not product_id:
                return make_json_error('Invalid product ID', 'validation_error')
            ...
            try:
                ...
                return make_json_error('Rating must be between 1 and 5', 'validation_error')
            ...
            if not Product.objects.filter(id=product_id, is_active=True).exists():
                return make_json_error('Product not found', 'not_found', status_code=404)
            
            if ProductReview.objects.filter(product_id=product_id, user_email=request.user.email).exists():
                return make_json_error('You have already reviewed this product', 'validation_error')
            ...
            for i, img in enumerate(request.FILES.getlist('images')[:self.MAX_IMAGES]):
                if img.size > self.MAX_IMAGE_SIZE:
                    return make_json_error(f'Image {i+1} exceeds 5MB limit', 'validation_error')
                if not img.content_type.startswith('image/'):
                    return make_json_error(f'File {i+1} is not a valid image', 'validation_error')
            ...
            return make_json_error('Failed to submit review', 'server_error', status_code=500)
...
class MarkReviewHelpfulView(View):
    ...
    def post(self, request):
        if not request.user.is_authenticated:
            return make_json_error('Please login', 'authentication_error', status_code=401)
        ...
        try:
            ...
            if not review_id:
                return make_json_error('Invalid review ID', 'validation_error')
            
            try:
                review = ProductReview.objects.get(id=review_id)
            except ProductReview.DoesNotExist:
                return make_json_error('Review not found', 'not_found', status_code=404)
            
            if review.user_email == request.user.email:
                return make_json_error('Cannot mark your own review', 'validation_error')
            ...
            return make_json_error('Already marked', 'validation_error')
                
        except ValueError as e:
            return make_json_error(str(e), 'validation_error')
        except Exception as e:
            logger.error(f"Mark helpful failed: {e}", exc_info=True)
            return make_json_error('Failed', 'server_error', status_code=500)
...
def get_product_reviews(request, product_id):
    ...
    try:
        ...
        logger.error(f"Get product reviews failed: {e}", exc_info=True)
        return make_json_error('Failed to retrieve reviews', 'server_error', status_code=500)
